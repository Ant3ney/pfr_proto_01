...
| Diagnose trainers that are inert only in a release/web export while authored routes, gyms, or the Champion challenge contain nonfunctional opponents | [`web-export-trainer-behavior.md`](web-export-trainer-behavior.md) | Recognize an exported null trainer behavior, preserve the reusable controller repair boundary, and verify the packaged PCK rather than only source scenes |
