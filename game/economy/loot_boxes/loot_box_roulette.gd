class_name LootBoxRoulette
extends Control

## Blocking loot-box reveal. The prize is committed by ShopSystem before
...
func present(summary: Dictionary) -> bool:
	...
	var reel := ShopSystem.build_loot_box_reel(
		box_id,
		winner_id,
		REEL_SIZE,
		WINNER_INDEX
	...
	($Panel/Margin/Column/Heading as Label).text = "%s  •  %s" % [
		...
		EconomySystem.format_money(price),
	]
...
func _add_reel_card(pokemon_id: int) -> void:
	var pokemon := ShopSystem.get_pokemon_offer(pokemon_id)
