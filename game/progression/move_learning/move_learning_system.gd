...
const UI_SCENE: PackedScene = preload(
	"res://game/progression/move_learning/move_learning_ui.tscn"
)
...
func _ensure_ui() -> MoveLearningUI:
	if is_instance_valid(_active_ui):
		return _active_ui
	_active_ui = UI_SCENE.instantiate() as MoveLearningUI
	if _active_ui != null:
