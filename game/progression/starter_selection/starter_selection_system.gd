...
const UI_SCENE: PackedScene = preload(
	"res://game/progression/starter_selection/starter_selection_ui.tscn"
)
...
var _active_ui: StarterSelectionUI
...
func get_active_ui() -> StarterSelectionUI:
...
func _ensure_ui() -> StarterSelectionUI:
	if is_instance_valid(_active_ui):
		return _active_ui
	_active_ui = UI_SCENE.instantiate() as StarterSelectionUI
	if _active_ui == null:
		return null
