...
## Player menu backed by the collection, inventory, and economy domains.
...
func stage_json_import(source: String, source_name := "selected JSON file") -> bool:
	if source.to_utf8_buffer().size() > ProgressionAutosaveService.MAX_JSON_TRANSFER_BYTES:
		_save_data_status.text = (
...
func give_xp_share_to_pokemon(pcl_id: String) -> bool:
	var result := InventorySystem.give_item_to_pokemon(
		InventorySystem.XP_SHARE_ITEM_KEY,
		pcl_id
	)
...
func take_held_item_from_pokemon(pcl_id: String) -> bool:
	var pcl := CollectionSystem.get_pcl(pcl_id)
	var pokemon_name := _pokemon_name(int(pcl.get("pokemonId", 0)))
	var result := InventorySystem.take_held_item_from_pokemon(pcl_id)
	if not bool(result.get("ok", false)):
...
func _build_interface() -> void:
	...
	_balance.text = EconomySystem.format_money(EconomySystem.get_balance())
...
func _on_import_file_selected(path: String) -> void:
	...
	if file == null:
		...
		return
	if file.get_length() > ProgressionAutosaveService.MAX_JSON_TRANSFER_BYTES:
		_save_data_status.text = (
			"That file is larger than the 2 MiB cloud-save limit and was not opened."
		)
		return
...
func _on_web_import_file_selected(arguments: Array) -> void:
	...
	if selected_file == null:
		...
		return
	if int(selected_file.size) > ProgressionAutosaveService.MAX_JSON_TRANSFER_BYTES:
		_save_data_status.text = (
			"That file is larger than the 2 MiB cloud-save limit and was not opened."
		)
		return
...
func _build_pokedex_entries() -> Array[Dictionary]:
	...
	var entries: Array[Dictionary] = []
	for offer in ShopSystem.get_pokemon_catalog():
...
func _configure_party_actions() -> void:
	...
	_held_item_action.visible = (
		...
		or InventorySystem.get_item_count(InventorySystem.XP_SHARE_ITEM_KEY) > 0
	)
...
func _confirm_discard() -> void:
	if _pending_discard_key.is_empty() or _pending_discard_quantity <= 0:
		return
	var result := InventorySystem.discard_item(
...
func _collection_details(entry: Dictionary) -> String:
	...
	var held_effect_text := (
		...
		if held_item_key == InventorySystem.XP_SHARE_ITEM_KEY
		else ""
	)
...
func _pokemon_name(pokemon_id: int) -> String:
	var offer := ShopSystem.get_pokemon_offer(pokemon_id)
...
func _update_summary() -> void:
	...
	var item_stacks := InventorySystem.get_item_inventory().size()
...
func _on_balance_changed(balance: int) -> void:
	_balance.text = EconomySystem.format_money(balance)
