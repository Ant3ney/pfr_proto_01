class_name AdventureMenu
extends Control

## Keyboard/gamepad/touch menu for shops and standalone-area travel.
...
var _closing := false
var _loot_box_roulette: LootBoxRoulette
...
func _ready() -> void:
	...
	if not _sprite_catalog.initialize():
		push_error("Adventure Menu Pokemon art could not initialize: %s" % _sprite_catalog.get_last_error())
	_build_interface()
	EconomySystem.balance_changed.connect(_on_balance_changed)
...
func _build_interface() -> void:
	...
	_balance_label.text = EconomySystem.format_money(EconomySystem.get_balance())
	...
	_status = Label.new()
	_status.name = "Status"
	_status.text = "Choose a shop offer or an adventure destination."
...
func _configure_filters() -> void:
	...
	if _category == CATEGORY_ITEMS:
		...
		for entry in ShopSystem.get_item_catalog():
		...
		for entry in ShopSystem.get_pokemon_catalog():
...
func _refresh_entries(focus_results := true) -> void:
	...
	match _category:
		CATEGORY_ITEMS:
			source = ShopSystem.get_item_catalog()
		CATEGORY_POKEMON:
			source = ShopSystem.get_pokemon_catalog()
		CATEGORY_LOOT_BOXES:
			source = ShopSystem.get_loot_box_catalog()
		CATEGORY_GYMS:
			source = ChallengeProgressionSystem.get_destinations("gym")
		CATEGORY_CHAMPION:
			source = ChallengeProgressionSystem.get_destinations("champion")
		CATEGORY_ROUTES:
			source = ChallengeProgressionSystem.get_destinations("route")
		_:
			source = []
...
func _entry_text(entry: Dictionary) -> String:
	match _category:
		CATEGORY_ITEMS:
			var owned := InventorySystem.get_item_count(String(entry.get("key", entry.get("slug", ""))))
			return "%s  —  %s  —  owned %d" % [
		...
		CATEGORY_POKEMON:
			return "#%04d  %s  [Lv. %d · %s]  —  %s" % [
				...
				ShopSystem.get_pokemon_purchase_level(
					int(entry.get("id", 0))
				),
		...
		CATEGORY_GYMS:
			var badge := "  ✓ BADGE EARNED" if ChallengeProgressionSystem.has_badge(int(entry.get("index", 0))) else ""
			return "%s%s" % [String(entry.get("name", "Gym")), badge]
		CATEGORY_CHAMPION:
			var clear := "  ✓ CLEARED" if ChallengeProgressionSystem.is_champion_completed() else ""
...
func _activate_selected() -> void:
	...
	match _category:
		CATEGORY_ITEMS:
			var result := ShopSystem.buy_item(
				String(_selected_entry.get("key", _selected_entry.get("slug", "")))
			)
			_show_purchase_result(result)
		CATEGORY_POKEMON:
			var result := ShopSystem.buy_pokemon(int(_selected_entry.get("id", 0)))
			_show_purchase_result(result)
		CATEGORY_LOOT_BOXES:
			var result := ShopSystem.buy_loot_box(String(_selected_entry.get("id", "")))
		...
		CATEGORY_ROUTES:
			if not bool(_selected_entry.get("unlocked", false)):
				...
				return
			_launch_selected_area()
		CATEGORY_GYMS, CATEGORY_CHAMPION:
			_launch_selected_area()


func _launch_selected_area() -> void:
	var area_id := String(_selected_entry.get("area_id", "")).strip_edges()
	var area := ChallengeProgressionSystem.get_area(area_id)
	if area == null:
		_status.text = "That destination is unavailable."
		return
	if not ChallengeProgressionSystem.is_area_unlocked(area_id):
		_status.text = ChallengeProgressionSystem.get_last_error()
		if _status.text.is_empty():
			_status.text = "That destination is locked."
		return
	# The NPC owns the movement lock. Notify it before transfer_to_scene()
	# snapshots movement state, then let the progression domain launch directly.
	_closing = true
	closed.emit()
	if not ChallengeProgressionSystem.launch_area(area_id):
		push_error(ChallengeProgressionSystem.get_last_error())
	queue_free()
...
func _show_purchase_result(result: Dictionary) -> void:
	if bool(result.get("ok", false)):
		...
		_status.text = "Purchased %s for %s." % [
			...
			EconomySystem.format_money(int(summary.get("price", 0))),
		]
	else:
...
func _item_details(entry: Dictionary) -> String:
	var owned := InventorySystem.get_item_count(
...
func _pokemon_details(entry: Dictionary) -> String:
	...
	return (
		...
		+ "[b]Appeal markup:[/b] %s\n[b]Catalog appeal score:[/b] %d / 100\n"
		...
		+ "and the catalog's appeal markup drive premium prices."
	) % [
		int(entry.get("id", 0)),
		String(entry.get("name", "Pokemon")),
		EconomySystem.format_money(int(entry.get("price", 0))),
		...
		ShopSystem.get_pokemon_purchase_level(int(entry.get("id", 0))),
	]
...
func _open_loot_box_result(summary: Dictionary) -> void:
	_loot_box_roulette = LootBoxScene.instantiate() as LootBoxRoulette
...
func _on_balance_changed(balance: int) -> void:
	if is_instance_valid(_balance_label):
		_balance_label.text = EconomySystem.format_money(balance)
