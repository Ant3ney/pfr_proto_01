...
func _run() -> void:
	...
	_check(
		...
		== CloudSaveSyncService.NATIVE_DEFAULT_ENDPOINT,
	...
	var cloud_economy := cloud_payload.get("economy", {}) as Dictionary
	cloud_economy["balance"] = 4321
	cloud_payload["economy"] = cloud_economy
	_touch_section(cloud_payload, "economy", 100)
	...
	_check(
		...
		and EconomySystem.get_balance() != 4321,
	...
	_check(
		EconomySystem.get_balance() == 4321
	...
	var portable_economy := portable_payload.get("economy", {}) as Dictionary
	portable_economy["balance"] = 5432
	portable_payload["economy"] = portable_economy
	...
	_check(
		...
		and EconomySystem.get_balance() == 5432
	...
	var second_cloud_economy := (
		second_cloud_payload.get("economy", {}) as Dictionary
	)
	second_cloud_economy["balance"] = 8765
	second_cloud_payload["economy"] = second_cloud_economy
	_touch_section(second_cloud_payload, "economy", 200)
	CloudSaveSync._on_request_completed(
		HTTPRequest.RESULT_SUCCESS,
		200,
		PackedStringArray(),
	...
	_check(
		...
		and EconomySystem.get_balance() == 8765
