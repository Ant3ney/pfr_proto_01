...
const TEST_SAVE_PATH := "user://pfr_progression_smoke_test.json"
const TEST_EXPORT_PATH := "user://pfr_progression_export_smoke_test.json"
...
func _run() -> void:
	...
	var saved_economy := original_economy.duplicate(true)
	saved_economy["balance"] = 777
	var saved_inventory := {
		"item_quantities": {"potion": 2},
		"claimed_gifts": ["autosave-exp-share-fixture"],
	}
	var saved_challenges := original_challenges.duplicate(true)
	saved_challenges["completed_routes"] = [0]
	saved_challenges["active_area_id"] = ""
	saved_challenges["run_defeated_ids"] = []
	_check(
		EconomySystem.load_save_data(saved_economy)
		and InventorySystem.load_save_data(saved_inventory)
		and ChallengeProgressionSystem.load_save_data(saved_challenges),
		"The save fixture should be able to set the independent domain state."
	)
	...
	_check(
		CollectionSystem.set_held_item(pcl_id, InventoryService.XP_SHARE_ITEM_KEY),
	...
	_check(
		...
		"The autosave owner should write a checkpoint immediately."
	)
	...
	_check(
		int(timestamped_payload.get("schema_version", 0)) == 6
		...
		and int(section_timestamps.get("economy", 0)) > 0
		and int(section_timestamps.get("inventory", 0)) > 0
		and int(section_timestamps.get("challenge_progression", 0)) > 0
		and int(section_timestamps.get("world", 0)) > 0,
		"Schema 6 should retain offline-safe timestamps for every mergeable save section."
	)
	...
	var changed_economy := saved_economy.duplicate(true)
	changed_economy["balance"] = 123
	EconomySystem.load_save_data(changed_economy)
	InventorySystem.reset_progress()
	ChallengeProgressionSystem.reset_progress()
	_check(
		ProgressionAutosave.load_now(),
		"The autosave owner should reload its saved checkpoint."
	)
	...
	_check(
		...
		and MoveLearningSystem.get_pending_requests().size() == 1
		and String(
			MoveLearningSystem.get_pending_requests()[0].get("moveId", "")
		) == "slash",
	...
	_check(
		EconomySystem.get_balance() == 777,
		"Loading should restore the economy balance alongside collection progression."
	)
	_check(
		InventorySystem.get_item_count("potion") == 2,
		"Loading should restore the player-menu Bag from the persistent item inventory."
	)
	_check(
		CollectionSystem.get_held_item(pcl_id) == InventoryService.XP_SHARE_ITEM_KEY,
		"Loading should restore the Pokemon's held item from its PCL."
	)
	_check(
		InventorySystem.has_claimed_gift("autosave-exp-share-fixture"),
		"Loading should restore one-time world gift claims."
	)
	_check(
		ChallengeProgressionSystem.get_completed_routes() == [0]
		and ChallengeProgressionSystem.is_route_unlocked(1),
	...
	cloud_payload["economy"]["balance"] = 888
	...
	cloud_timestamps["economy"] = int(cloud_timestamps.get("economy", 0)) + 100
	...
	_check(
		...
		and EconomySystem.get_balance() == 888,
	...
	_check(
		...
		== ProgressionAutosaveService.SAVE_SCHEMA_VERSION
		and exported_payload.has("profile")
		and exported_payload.has("collection")
		and exported_payload.has("move_learning")
		and exported_payload.has("economy")
		and exported_payload.has("inventory")
		and exported_payload.has("challenge_progression")
		...
		"JSON export should use the credential-free schema-6 cloud progression payload."
	)
	_check(
	...
	var before_import_economy := EconomySystem.get_save_data()
	before_import_economy["balance"] = 321
	_check(
		EconomySystem.load_save_data(before_import_economy),
	...
	_check(
		...
		and int(imported_timestamps.get("economy", 0))
		== int(imported_meta.get("saved_at_ms", 0)),
	...
	_check(
		...
		and int(
			((checkpoint_value as Dictionary).get("economy", {}) as Dictionary).get(
				"balance",
				-1
			)
		) == 888,
	...
	if _failures.is_empty():
		print(
			"Progression autosave smoke test passed: disk checkpoint, validated "
	...
	for failure in _failures:
		push_error("Progression autosave smoke test failed: %s" % failure)
	get_tree().quit(1)
