...
class TransferWatcher:
	...
	const ROUTES := [
		{
			...
			"destination": "res://game/world/levels/new_bouffalant_city/interiors/pokemon_center/pokemon_center_interior.tscn",
			"spawn": "EntrySpawn",
		...
		},
		{
			...
			"destination": "res://game/world/levels/new_bouffalant_city/interiors/pokemon_center/pokemon_center_annex.tscn",
			"spawn": "EntrySpawn",
		...
		},
		{
			...
			"destination": "res://game/world/levels/new_bouffalant_city/interiors/city_hall_interior.tscn",
		...
		},
		{
			...
			"destination": "res://game/world/levels/new_bouffalant_city/interiors/miare_station_concourse.tscn",
			"spawn": "EntrySpawn",
		...
		},
		{
			...
			"destination": "res://game/world/levels/new_bouffalant_city/interiors/gatehouse_interior.tscn",
			"spawn": "FrontEntrySpawn",
		...
		},
		{
			...
			"destination": "res://game/world/levels/new_bouffalant_city/interiors/west_tenant_lobby.tscn",
			"spawn": "EntrySpawn",
		...
		},
		{
			...
			"destination": "res://game/world/levels/new_bouffalant_city/interiors/museum_gallery.tscn",
			"spawn": "EntrySpawn",
			"return": "MuseumReturn",
			"exit": "ExitToCity",
		},
		{
			...
			"destination": "res://game/world/levels/new_bouffalant_city/interiors/north_tenant_lobby.tscn",
			"spawn": "EntrySpawn",
	...
	func run() -> void:
		...
		for removed_name in REMOVED_CITY_TRANSITIONS:
			_check(
				_find_node_by_name(city, removed_name) == null,
		...
		for route: Dictionary in ROUTES:
			var trigger := _find_node_by_name(city, StringName(route.trigger)) as SceneTransferTrigger
		...
		for route: Dictionary in ROUTES:
			...
			if not String(route.trigger).begins_with("PokemonCenter"):
				var return_marker := _find_node_by_name(city, StringName(route.return)) as Marker3D
		...
		var station_trigger := _find_node_by_name(city, &"MiareStationEntranceTrigger") as SceneTransferTrigger
	...
	func _validate_red_city_barriers(city: Node) -> void:
		for barrier_name in RED_CITY_BARRIERS:
			var barrier := _find_node_by_name(city, barrier_name) as MeshInstance3D
			_check(
				barrier != null,
	...
	func _validate_route_configuration(
		...
		var return_marker := _find_node_by_name(city, return_name) as Marker3D
	...
	func _exercise_photographed_station_route(
		...
		const STATION_PATH := "res://game/world/levels/new_bouffalant_city/interiors/miare_station_concourse.tscn"
		...
		_check(
			...
			and stretchman.controller.npc_behavior is MenuNpcBehavior,
			"Miare Station should contain Stretchman as an ordinary menu NPC."
		)
		_check(
			stretchman_return != null,
			"Miare Station should retain the Adventure Menu destination return marker."
		)
		...
		var return_marker := _find_node_by_name(city, &"MiareStationReturn") as Marker3D
	...
	func _finish() -> void:
		if failures.is_empty():
			print(
				...
				+ "contains Stretchman as a generic menu NPC."
			)
			get_tree().quit(0)
			return
