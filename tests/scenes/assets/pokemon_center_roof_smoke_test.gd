...
const WRAPPER_SCENE := preload("res://game/world/level_kits/structures/pokemon_center/pokemon_center_with_roof.tscn")
const NEW_BOUFFALANT_CITY_SCENE := preload("res://game/world/levels/new_bouffalant_city/new_bouffalant_city.tscn")
const WRAPPER_PATH := "res://game/world/level_kits/structures/pokemon_center/pokemon_center_with_roof.tscn"
...
func _ready() -> void:
	...
	await get_tree().process_frame
	var placed_center := new_bouffalant_city.get_node_or_null(
		^"NavigationRegion3D/WorldGeometry/Structures/T1BPokemonCenterOut"
	) as Node3D
	...
	if (
		...
		return
	var sun := new_bouffalant_city.get_node_or_null(^"Environment/Sun") as DirectionalLight3D
