...
func _ready() -> void:
	...
	var environment_root := annex.get_node_or_null(
	...
	var collision_root := annex.get_node_or_null(
	...
	for node_path: String in [
		...
		if _find_node_by_name(annex, StringName(node_path)) == null:
			_fail("Pokemon Center annex is missing required node %s." % node_path)
			return

	var exit_trigger := _find_node_by_name(annex, &"ExitToCity") as SceneTransferTrigger
	if (
		exit_trigger == null
		...
		return
	var key_light := annex.get_node_or_null(^"Environment/KeyLight") as DirectionalLight3D
	var fill_a := annex.get_node_or_null(^"Environment/DispensaryFill") as OmniLight3D
	var fill_b := annex.get_node_or_null(^"Environment/ConsultationFill") as OmniLight3D
