...
class TransferWatcher:
	...
	const MAIN_INTERIOR_PATH := "res://game/world/levels/new_bouffalant_city/interiors/pokemon_center/pokemon_center_interior.tscn"
	const EAST_ANNEX_PATH := "res://game/world/levels/new_bouffalant_city/interiors/pokemon_center/pokemon_center_annex.tscn"
	...
	func run() -> void:
		...
		var south_marker := _find_node_by_name(city, &"PokemonCenterEntrance") as Marker3D
		var east_marker := _find_node_by_name(city, &"PokemonCenterEastEntrance") as Marker3D
		_check(
		...
		var south_trigger := _find_node_by_name(city, &"PokemonCenterSouthEntranceTrigger") as SceneTransferTrigger
		var east_trigger := _find_node_by_name(city, &"PokemonCenterEastEntranceTrigger") as SceneTransferTrigger
	...
	func _exercise_route(
		...
		if city == null or city.scene_file_path != CITY_PATH:
			return
		var city_player := _find_player_character(city)
		var entrance_trigger := _find_node_by_name(city, StringName(trigger_name)) as SceneTransferTrigger
		...
		var entry_spawn := _find_node_by_name(interior, &"EntrySpawn") as Marker3D
		_check(
		...
		var exit_trigger := _find_node_by_name(interior, &"ExitToCity") as SceneTransferTrigger
		...
		var return_marker := _find_node_by_name(city, return_marker_name) as Marker3D
		_check(
