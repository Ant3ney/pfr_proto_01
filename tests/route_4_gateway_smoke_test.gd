...
class GatewayWatcher:
	...
	const ROUTE_PATH := "res://overworld/route_0/route_0.tscn"
	...
	func run() -> void:
		...
		var city_entrance := city.get_node_or_null(
			^"GateBuildingFrontEntranceTrigger"
		) as SceneTransferTrigger
		var gate_model := city.get_node_or_null(
			^"NewBouffalantCityAssets2/T1BGateBuilding"
		) as Node3D
		...
		var exit_room := gatehouse.get_node_or_null(^"Route0ExitRoom") as Node3D
		var route_exit := gatehouse.get_node_or_null(
			^"Route0ExitRoom/ExitToRoute0"
		) as SceneTransferTrigger
		var return_spawn := gatehouse.get_node_or_null(
			^"Route0ExitRoom/Route0ReturnSpawn"
		) as Marker3D
		var player := gatehouse.get_node_or_null(^"Runtime/Player") as PlayerCharacter
		var front_entry := gatehouse.get_node_or_null(^"FrontEntrySpawn") as Marker3D
		var front_exit := gatehouse.get_node_or_null(^"ExitFront") as SceneTransferTrigger
		var city_exit_label := gatehouse.get_node_or_null(^"CityExitLabel") as Label3D
		var city_exit_guide := gatehouse.get_node_or_null(^"CityExitGuide") as MeshInstance3D
		var route_exit_label := gatehouse.get_node_or_null(
			^"Route0ExitRoom/Route0DoorLabel"
		) as Label3D
		var route_exit_guide := gatehouse.get_node_or_null(
			^"Route0ExitRoom/Route0DoorGuide"
		) as MeshInstance3D
		...
		_check(
			gatehouse.get_node_or_null(^"Route0ExitRoom/Route0Gateway") == null
			and gatehouse.get_node_or_null(^"Route0ExitRoom/Route4Gateway") == null,
		...
		if route == null or route.scene_file_path != ROUTE_PATH:
			_finish()
			return
		var trainer_root := route.get_node_or_null(^"RouteTrainers") as Node3D
		...
		var route_start := route.get_node_or_null(^"Route0Start") as Marker3D
