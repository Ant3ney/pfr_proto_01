...
const INTERIOR_SCENE := preload("res://art/environments/new_bouffalant_city/pokemon_center_interior/pokemon_center_interior.tscn")
...
func _ready() -> void:
	...
	var environment_root := interior.get_node_or_null("Environment") as Node3D
	...
	var collision_root := interior.get_node_or_null("Collision") as Node3D
	...
	for node_path: String in required_nodes:
		if interior.get_node_or_null(node_path) == null:
			_fail("Pokemon Center interior is missing required node %s." % node_path)
			return

	var key_light := interior.get_node_or_null("KeyLight") as DirectionalLight3D
	var fill_left := interior.get_node_or_null("CounterFillLeft") as OmniLight3D
	var fill_right := interior.get_node_or_null("CounterFillRight") as OmniLight3D
	...
	var camera := interior.get_node_or_null("Camera") as Camera3D
	...
	var healer := interior.get_node_or_null("PokemonCenterHealer") as PFRCharacter
	var nurse_spawn := interior.get_node_or_null("NurseSpawn") as Marker3D
	if (
		healer == null
		or nurse_spawn == null
	...
	var counter_shape := interior.get_node_or_null(
		^"Collision/HealingCounter/CollisionShape3D"
	) as CollisionShape3D
	var console_shape := interior.get_node_or_null(
		^"Collision/HealingConsole/CollisionShape3D"
	) as CollisionShape3D
	if (
		healer_shape == null
