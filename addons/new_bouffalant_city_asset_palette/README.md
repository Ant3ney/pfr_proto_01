...
Museum and Gate Building are the two confirmed triggers for that driver hang. On Intel Vulkan, the palette pauses only those two entries, play from a scene containing one, and the metric browser because it contains both. High-load classification remains visible but does not block the other assets.

Close every editor instance for this project, then choose either validated launcher from the repository root. Intel Compatibility matches the project setting and is the simplest option:
