...
const MoveLearningUI := preload("res://rnd/move_learning/move_learning_ui.tscn")
...
func _test_replacement_ui() -> void:
	var ui := MoveLearningUI.instantiate() as RNDMoveLearningUI
	add_child(ui)
...
func _test_level_change_queue_and_choices() -> void:
	...
	var pending := RNDMoveLearningSystem.get_pending_requests()
	_check(
		pending.size() == 1
		and String(pending[0].get("moveId", "")) == "leechseed",
		"Crossing Bulbasaur's level 9 threshold should queue Leech Seed once."
	)
	var auto_result := RNDMoveLearningSystem.resolve_next_pending()
	_check(
	...
	var saved_pending := RNDMoveLearningSystem.get_save_data()
	_check(
	...
	RNDMoveLearningSystem.reset_for_testing()
	_check(
		RNDMoveLearningSystem.load_save_data(saved_pending),
	...
	RNDMoveLearningSystem.move_learning_resolved.connect(
	...
	var presentation_ok := await RNDMoveLearningSystem.present_pending_for_member(
		palkia_id
	)
	...
	var skip_result := RNDMoveLearningSystem.resolve_next_pending(-1)
	_check(
	...
	pending = RNDMoveLearningSystem.get_pending_requests()
	_check(
		pending.size() == 2
	...
	RNDMoveLearningSystem.resolve_next_pending(0)
	RNDMoveLearningSystem.resolve_next_pending(-1)
	_check(
		RNDMoveLearningSystem.get_pending_requests().is_empty(),
	...
	_check(
		not RNDMoveLearningSystem.validate_save_data(
			invalid_save,
...
func _drive_system_replacement_ui(
	...
	var ui := RNDMoveLearningSystem.get_node_or_null(^"MoveLearningUI") as RNDMoveLearningUI
