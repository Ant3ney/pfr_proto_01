...
func _run() -> void:
	...
	if hub != null:
		...
		search.text = "poton"
		await get_tree().process_frame
		_check(_list_contains(offers, "Potion"), "Elastic item search should recover a one-letter Potion typo.")
		...
		_check(offers.item_count == 8, "The route tab should render all route destinations.")
	...
	if _failures.is_empty():
		print(
			...
			+ "tabs, fuzzy filters, GIF art, loot reel, focus navigation, and cleanup verified."
		)
		get_tree().quit(0)
		return
