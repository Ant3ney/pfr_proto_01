...
const SOURCE_SCENE_PATH := "res://rnd/tests/battle_return_position_smoke_test.tscn"
...
class BattleReturnWatcher:
	...
	func run() -> void:
		...
		var accepted := GameInstance.startBattle({
			"encounter_type": "wild",
			"battle_scene_path": "res://battle/battle_scene.tscn",
		})
		_check(accepted, "The source scene should launch the shared battle scene.")
		await _wait_for_scene_path("res://battle/battle_scene.tscn")
		_check(
