class_name RNDRouteCompletionGate
extends Area3D

## Physical far-end checkpoint shared by the authored Route 0 and generated
## Routes 1–39. Winning the last fight is insufficient: the player must touch
## this goal before the next route appears in Stretchman's menu.
...
func configure(index: int) -> void:
	route_index = clampi(index, 0, RNDStretchContent.ROUTE_COUNT - 1)
	if is_instance_valid(_label):
		_update_label("ROUTE %d END" % route_index)


func attempt_completion() -> Dictionary:
	var result := StretchGoalSystem.complete_route_at_end(route_index)
